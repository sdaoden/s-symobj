...
{package X3;
	our @ISA = ('X2');
	SymObj::sym_create(0, {_name => 'X3 override', '@_array2' => undef, '%_hash2' => undef})
}
...
ok($o->name eq 'EASY T3' && $o->hash->{newhk1} eq 'newhv1' && $o->hash->{newhk2} eq 'newhv2');
...
$o = TX->new;
ok($m == 0b00111111);
ok($o->n eq 'TX' && $o->v == 1000 &&
	$o->i1 eq 'T1_0' && $o->i2 eq 'T1_1' && $o->i3 eq 'T1_2' &&
	$o->i4 eq 'T2_0' && $o->i5 eq 'T2_1');
